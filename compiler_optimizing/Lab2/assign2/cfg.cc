...
void cfg::build_cfg(simple_instr *inlist_)
{
	inlist = inlist_;
	build_cfg_helper();
}
...
void cfg::process_loops()
{
	...
	for (i = 0; i < natloop_num; i++)
	{
		natloop * newloop = new natloop(this, loop_heads[i], back_edge[loop_heads[i]], i);
	...
	std::set<int> & vistedSet, int v)
{
	...
	//cout << "start visiting "<< v << endl;
	int_type successor = vertexlist[v]->get_succ();
	int out_degree = successor.size();

	//cout << "start visiting "<< v << " with out_degree" << out_degree <<endl;
